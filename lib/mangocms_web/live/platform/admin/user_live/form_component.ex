defmodule MangoCMSWeb.Platform.Admin.UserLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <section class="rounded-lg border border-base-300 bg-base-100 p-6 text-base-content shadow-sm transition-colors">
      <.header>
        {@title}
        <:subtitle>
          Create platform operators and customer accounts. Password is required only for new users.
        </:subtitle>
      </.header>
      ...
      >
        ...
        <div class="mt-6 flex items-center justify-end gap-3">
          ...
          <.button id="save-platform-user-button" variant="primary" phx-disable-with="Saving...">
            Save user
          </.button>
        </div>
      </.form>
  ...
  def update(%{user: user} = assigns, socket) do
    ...
    {:ok,
     socket
     |> assign(assigns)
     |> assign(:role_options, Authorization.platform_role_options())
  ...
  defp save_user(socket, :edit, user_params) do
    case Accounts.update_user(socket.assigns.user, user_params) do
      {:ok, user} ->
        ...
        {:noreply,
         socket
         |> put_flash(:info, "Platform user updated successfully")
  ...
  defp save_user(socket, :new, user_params) do
    case Accounts.create_user(user_params) do
      {:ok, user} ->
        ...
        {:noreply,
         socket
         |> put_flash(:info, "Platform user created successfully")
         |> push_patch(to: socket.assigns.patch)}
